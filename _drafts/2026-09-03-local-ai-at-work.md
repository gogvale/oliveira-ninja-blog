...
They were right. An unreviewed third-party tool gets whatever context you feed it, and saying yes without a policy is how client data leaves the building through a chat window.

We wanted the tool. The approved path did not exist, and nobody offered an alternative. For one honest moment, the shortest path looked like running our own AI for the demo. No approval. No policy. No governance. Just a model and a task.
...
This is not a post about a bad security team. The security team was right.

An MCP server is a standing channel into your context — the vendor's tooling, the client's data, your prompts, your code. Approving one without a policy means no review, and review exists because vendors differ. Some are careful. Some are not. You can not tell from the marketing page.
...
The pattern is older than AI. Security blocks something to protect the company, and the block lands on an employee with a job to do. The employee does not fight the policy — they find the shortest path around it.
...
The shadow AI literature stops at the same place every time: do not ban, give employees secure tools they want to use. Fair. Now name the tool. Nobody does, because the honest answer is uncomfortable.

A cloud tool needs the slow review — data flows, vendor audit, contractual liability. And capability climbs every release while the policy sits still, so an approved cloud tool arrives one release behind.
...
The local-model threads prove the same thing: usable quality keeps landing on smaller hardware. A model that needed a rented GPU not long ago runs on a developer's laptop. No server. No per-seat cost. No vendor in the path. For our demo, it would have been enough — and the demo was the whole point.
...
The checklist exists because **the model is not the boundary — the tools are**. A local model with agentic tool access can still exfiltrate through the terminal it is allowed to invoke, prompt injection or not. That is why the policy constrains tools, not just ports.
...
We did not build the shadow demo — nobody has to. A policy that says yes to the model that can not leave turns a reasonable no into a governed yes in a week, and the route can be a laptop with the network turned off.
